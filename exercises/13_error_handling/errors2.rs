...
fn total_cost(item_quantity: &str) -> Result<i32, ParseIntError> {
    ...
    // If the parsing was successful, we can use the value.
    // If it was not successful, we return the error.
    // This is the shorter way to do it:
    // let qty = item_quantity.parse::<i32>()?;
    // The `?` operator is a shorthand for the `match` statement below.
    // If the parsing was successful, we can use the value.
    // If it was not successful, we return the error.
